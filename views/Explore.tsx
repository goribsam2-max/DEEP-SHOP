    ...
    <div className="min-h-screen bg-white dark:bg-black animate-fade-in pb-40">
      ...
      <section className="sticky top-20 z-40 glass border-b border-slate-100 dark:border-white/5 px-4 md:px-12 py-8">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div>
            <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tight mb-2">Explore <span className="text-primary">Products</span></h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{filteredProducts.length} items found</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 flex-1 max-w-2xl">
            <div className="relative w-full">
              ...
              <input 
                type="text" 
                placeholder="Search gadgets, mobiles..." 
                className="w-full h-14 pl-14 pr-6 bg-slate-50 dark:bg-white/5 rounded-2xl outline-none font-bold text-sm border border-transparent focus:border-primary transition-all shadow-inner"
                value={searchQuery}
          ...
          <>
            {filteredProducts.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-40 opacity-20">
                ...
                <h3 className="text-xl font-black uppercase tracking-widest">No matching items found</h3>
              </div>
            )}
          </>
        )}
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <Link 
      ...
      className="group flex flex-col bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 rounded-[32px] overflow-hidden transition-all duration-500 hover:shadow-2xl h-full relative"
    >
